...
public class Tele extends OpModeCommand {
    ...
    public void initialize() {
        ...
        ButtonMapper mapper = new ButtonMapper()
                .put(gamepad_1.a.risingEdge(),
                        () -> new Instant(() -> TrackingThread.trackTurret = true))
                .put(gamepad_1.y.risingEdge(),
                        () -> new Instant(() -> tsh.setForce(!tsh.isForce())))
                .put(gamepad_1.dpad_up.risingEdge(),
                        () -> tsh.setting(robot::shootFar))
                .put(gamepad_1.dpad_down.risingEdge(),
                        () -> tsh.setting(robot::shootNear))
                .put(gamepad_1.dpad_left.risingEdge(),
                        () -> tsh.setting(robot::shootMedium))
                .put(gamepad_1.right_trigger.greaterThan(0.3f).risingEdge(),
                        () -> tsh.task(
                                () -> robot.turret.runToPos(
                                        robot.turret.getTargetPosition()
                                                + (int) (20 * gamepad1.right_trigger)
                                ),
                                new int[]{1, 1, 0}
                        )
                )
                .put(gamepad_1.left_trigger.greaterThan(0.3f).risingEdge(),
                        () -> tsh.task(
                                () -> robot.turret.runToPos(
                                        robot.turret.getTargetPosition()
                                                - (int) (20 * gamepad1.left_trigger)
                                ),
                                new int[]{1, 1, 0}
                        )
                )
                .put(gamepad_1.right_bumper.risingEdge(),
                        () -> tsh.task(robot.turret::next, new int[]{1, 1, 0}))
                .put(gamepad_1.left_bumper.risingEdge(),
                        () -> tsh.task(robot.turret::previous, new int[]{1, 1, 0}))
                .put(gamepad_1.b.risingEdge(),
                        () -> new Sequential(
                                new WaitUntil(robot.turret.limitSwitch::state),
                                new Instant(robot.turret::resetPosition)
                        )
                )
                .put(gamepad_1.x.risingEdge(),
                        () -> tsh.override(
                                robot.popper.neutral(),
                                RobotStateHandler.IntakeMessage.SORTING
                        )
                )
                .put(gamepad_2.b.risingEdge().and(() ->
                ...
                .put(gamepad_2.right_bumper.risingEdge(),
                        () -> tsh.task(
                                new Sequential(
                                        new Instant(() -> robot.setRobotState(
                                                RobotStateHandler.IntakeMessage.SORTING)),
                                        robot.table.next()
                                ),
                                new int[]{1, 0, 0}
                ...
                .put(gamepad_2.left_bumper.risingEdge(),
                        () -> tsh.task(
                                new Sequential(
                                        new Instant(() -> robot.setRobotState(
                                                RobotStateHandler.IntakeMessage.SORTING)),
                ...
                .put(gamepad_2.x.risingEdge().and(() ->
